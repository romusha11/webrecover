...
export default function Profile() {
  ...
  // Simulasi join date
  const joinDate = user.joinDate || "2023-01-15";
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-purple-50 py-8 px-2">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md border border-gray-200">
        <h2 className="text-2xl font-black mb-4 text-center text-blue-700">Profil Pengguna - Romusha</h2>
        <div className="flex flex-col items-center mb-6">
          ...
          {!editing ? (
            ...
            <form onSubmit={handleSave} className="w-full mt-2 space-y-3">
              ...
              <div>
                <label className="block text-sm font-bold mb-1">URL Avatar (gambar)</label>
                <input
                  type="url"
              ...
              <button
                ...
                onClick={() => { setEditing(false); setError(''); setSuccess(''); setAvatar(user.avatar || defaultAvatar); setName(user.name); setEmail(user.email); }}
              >
                Batal
              </button>
            </form>
        ...
        {success && <div className="text-green-600 mb-2 text-center font-bold">{success}</div>}
        {!editing && (
          <button
